...
import deletePostById from "../../utils/deletePostById";
export default function DeleteMenu({ setShowDeleteMenu, postID, author, setReRenderPost }) {
  ...
  function deletePost(id) {
    deletePostById(id)
      .then((res) => {
        ...
        if (setReRenderPost) setReRenderPost((old) => old + 1);
        else navigate(`/${profileData.username}`);
      })
      .catch((err) => {
        toast({
    ...
    <div className="fixed bottom-0 inset-x-0 pb-6 sm:inset-0 sm:p-0 sm:flex sm:items-center sm:justify-center z-10">
      ...
      <div className="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
        <div className="bg-white sm:p-6 sm:pb-4">
          <h1 className="text-center font-semibold pb-4">Delete Post</h1>
          <hr className="w-[100%]" />
        </div>
        <div className=" w-[100%]"></div>
        <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
          <span className="mt-3 flex w-full rounded-md shadow-sm sm:mt-0 sm:w-auto">
            {profileData._id === author._id && (
              <button
                onClick={() => {
                  deletePost(postID);
